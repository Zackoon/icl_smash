...
class MeleeDataset(Dataset):
    ...
    def __init__(self, data_path: str, match_id: int, 
                 # num_enums: int, 
                 all_inputs=None):
        """Initialize dataset.
        ...
        if data_path != INFERENCE_MODE:
            ...
            input_continuous = torch.tensor(all_inputs.astype(np.float32), dtype=torch.float32)
            ...
            target_continuous = torch.tensor(all_targets.astype(np.float32), dtype=torch.float32)
